...
def plot_value_function(value_function, maze, alpha, gamma, epsilon):
    ...
    ax.set_title(f"Value Function for alpha={alpha}, gamma={gamma}, epsilon={epsilon}")
    plt.show()

def plot_policy(value_function, maze, alpha, gamma, epsilon):
    ...
    for o in obs_position:
        ax.add_patch(plt.Rectangle(o[::-1], 1, 1, fill=True, edgecolor='black', facecolor='gray'))
    ax.set_title(f"Policy Map for alpha={alpha}, gamma={gamma}, epsilon={epsilon}")
    plt.show()
